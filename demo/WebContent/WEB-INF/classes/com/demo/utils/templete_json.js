var DWZ = {
    ...
    ajaxError: function(xhr, ajaxOptions, thrownError) {
        if (alertMsg) {
            alertMsg.error("<div>Http status: " + xhr.status + " " + xhr.statusText + "</div><div>ajaxOptions: " + ajaxOptions + "</div><div>thrownError: " + thrownError + "</div><div>" + DWZ.getHtmlBody(xhr.responseText) + "</div>")
        } else {
        ...
        ajaxUrl: function(op) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (json[DWZ.keys.statusCode] == DWZ.statusCode.error) {
                        if ($.pdialog) {
                            $.pdialog.checkCloseCurrent(json)
                        }
                        if (navTab) {
                            navTab.checkCloseCurrent(json)
                        }
...
function Map() {
    ...
    function replaceTmEval(data) {
        return data.replace(RegExp("({[A-Za-z0-9_+-]*})", "g"),
        function($1) {
                            ...
                            $("div.ckbox", $this).each(function() {
                                ...
                                ckbox.click(function() {
                                    ...
                                    tnode.parents("li").each(function() {
                                        var $pNode = $(this),
                                        $pCkbox = $pNode.find(">div>div.ckbox");
    ...
    $.dwz.accordion = function(container, options) {
        ...
        $.dwz.accordion.defaults, options);
...
function initUI($p) {
    ...
    if ($.fn.cssTable) {
        $("table.list", $p).not(".nowrap").cssTable()
    }
    if ($.fn.jPanel) {
        $("div.panel", $p).jPanel()
    }
    ...
    if ($.fn.datepicker) {
        $("input.date", $p).each(function() {
            ...
            if ($this.attr("defaultTime")) {
                opts.defaultTime = $this.attr("defaultTime")
            }
    ...
    if ($.fn.selectedBlank) {
        $("a[target=selectedBlank]", $p).selectedBlank()
    }
    if ($.fn.pagerForm) {
...
var navTab = {
    ...
    _scrollCurrent: function() {
        ...
        if (iW <= this._getScrollBarW()) {
            ...
            if (this._getLeft() < this._getScrollBarW() - iW) {
                this._scrollTab(this._getScrollBarW() - iW)
            } else {
                if (this._currentIndex < this._visibleStart()) {
                    this._scrollTab( - this._getTabsW(0, this._currentIndex))
                } else {
                    if (this._currentIndex >= this._visibleEnd()) {
    ...
    checkCloseCurrent: function(json) {
        ...
        if (json[DWZ.keys.statusCode] == DWZ.statusCode.timeout || (json[DWZ.keys.statusCode] == DWZ.statusCode.error && "closeCurrentNavTab" == json.callbackType)) {
            this.closeCurrentTab()
        }
    },
                    ...
                    $("a", this).each(function() {
                        ...
                        if ($link.hasClass(op.ajaxClass)) {
                            $link.click(function(event) {
                                ...
                                if (this.href && ($(this).attr("data-cache") == "false" || !jGroup.attr("loaded"))) {
                                    jGroup.loadUrl(this.href, {},
                                    function() {
                            ...
                            if ($link.hasClass(op.closeClass)) {
                                $link.click(function(event) {
    ...
    $.pdialog = {
        ...
        checkCloseCurrent: function(json) {
            ...
            if (json[DWZ.keys.statusCode] == DWZ.statusCode.timeout || (json[DWZ.keys.statusCode] == DWZ.statusCode.error && "closeCurrentDialog" == json.callbackType)) {
                this.closeCurrent()
            }
        },
    ...
    DWZ.sortDrag = {
        ...
        drag: function(el, event) {
            ...
            if ($sortBox.attr("data-over-sort") == "true" && $overBox.length > 0 && $overBox[0] != $sortBox[0]) {
    ...
    $.taskBar = {
        ...
        _scrollTask: function(task) {
            ...
            if (task.position().left + this._getLeft() + task.outerWidth() > this._getBarWidth()) {
                ...
                if (task.position().left + this._getLeft() < 0) {
    ...
    if (json[DWZ.keys.statusCode] == DWZ.statusCode.ok) {
        ...
        if ("closeCurrent" == json.callbackType) {
            ...
            if ("forward" == json.callbackType) {
                ...
                if ("forwardConfirm" == json.callbackType) {
    ...
    $.fn.datepicker = function(opts) {
        ...
        return this.each(function() {
            ...
            $this.click(function(event) {
                ...
                $.each($.regional.datepicker.monthNames,
                function(i, v) {
                    var m = i + 1;
    ...
    $.extend($.fn, {
        jPanel: function(options) {
            ...
            return this.each(function() {
                ...
                if (collapse) {
                    $('<a href="javascript:"></a>').addClass(close ? op.exp: op.coll).insertAfter(title)
                }
